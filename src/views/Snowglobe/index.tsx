...
function Stake() {
  ...
  const roi_day =
    (1 + parseFloat(rewardYield) / 100) ** (duration * epochs_per_day) - 1;
  ...
  const futureWealth =
    (parseFloat(ssbQuantity) + gains) * parseFloat(futurePrice);
          ...
          >
            <Grid item>
              <div className="snowglobe-card-header">
                ...
                <p className="snowglobe-card-header-subtitle">
                  Estimate your returns
                </p>
                <p className="snowglobe-card-header-disclaimer">
                  Any financial projections or returns shown on the calculator
                  are estimated predictions of returns only, are hypothetical,
                  are not based on actual investment results and are not
                  guarantees of future results. Estimated projections do not
                  represent or guarantee the actual results of any transaction,
                  and no representation is made that any transaction will, or is
                  likely to, achieve results or profits similar to those shown.
                </p>
              </div>
            </Grid>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={12} sm={4} md={4} lg={4}>
                    <div className="snowglobe-card-apy">
                      <p className="snowglobe-card-metrics-title">
                        Current SB Price
                      </p>
                      <p className="snowglobe-card-metrics-value">
                        {isAppLoading ? (
            ...
            <div className="snowglobe-card-area">
              {!address && (
                <div className="snowglobe-card-wallet-notification">
                  <div
                    className="snowglobe-card-wallet-connect-btn"
                    onClick={connect}
                  >
                    <p>Connect Wallet</p>
                  </div>
                  <p className="snowglobe-card-wallet-desc-text">
                    Connect your wallet to use the Snowglobe
                  </p>
                </div>
              )}
              {address && (
                    ...
                    <Grid className="snowglobe-card-grid" container spacing={1}>
                      <Grid item sm={5} className="snowglobe-entry">
                        <div className="snowglobe-card-action-row">
                          <p className="snowglobe-card-action-label">
                            sSB AMOUNT
                          </p>
                          <OutlinedInput
                            type="number"
                            ...
                            endAdornment={
                              <InputAdornment position="end">
                                <div
                                  onClick={setSsbToMax}
                                  className="snowglobe-card-action-input-btn"
                                >
                                  <p>Max</p>
                                </div>
                              </InputAdornment>
                        ...
                        <div className="snowglobe-card-action-row">
                          ...
                          <OutlinedInput
                            ...
                            endAdornment={
                              <InputAdornment position="end">
                                <div
                                  onClick={setCurrentPriceToFuture}
                                  className="snowglobe-card-action-input-btn"
                                >
                                  <p>CURRENT</p>
                                </div>
