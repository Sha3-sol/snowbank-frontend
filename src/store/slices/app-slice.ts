...
export const loadAppDetails = createAsyncThunk(
  ...
  async ({ networkID, provider }: ILoadAppDetails) => {
    ...
    const circSupply =
      (await ssbContract.circulatingSupply()) / Math.pow(10, 9);
    ...
    const sbBondsAmountsPromises = allBonds.map((bond) =>
      bond.getSbAmount(networkID, provider)
    );
    const sbBondsAmounts = await Promise.all(sbBondsAmountsPromises);
    const sbAmount = sbBondsAmounts.reduce(
      (sbAmount0, sbAmount1) => sbAmount0 + sbAmount1,
      0
    );
